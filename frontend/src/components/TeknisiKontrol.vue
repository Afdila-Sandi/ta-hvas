...
// Mengimpor data yang kini diurus sepenuhnya oleh ws.js dan ESP32
import { sensorData } from "../services/ws";

const isSending = ref(false);
const modeKendali = ref("manual");

// Pre-fill durasi sesuai SOP: 3 Jam (180 menit) ON, 1 Jam (60 Menit) OFF
const inputDurasiOn = ref(180);
const inputDurasiOff = ref(60);

// Mengubah format menit dari ESP32 menjadi "X Jam Y Menit"
...
const kirimJadwalSiklus = async () => {
  ...
  isSending.value = true;
  try {
    // Mengirim perintah siklus ke API Gateway yang akan diteruskan ke ESP32
    ...
    // Opsional: Langsung ubah UI ke mode berjalan agar tidak terasa delay
